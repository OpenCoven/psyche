...
impl Store {
    ...
    pub fn load(
        ...
        if document.schema_version().kind != kind {
            return Err(StoreError::Contract(ContractError::SchemaMismatch {
                expected: kind,
                found: document.schema_version().kind,
            }));
        }
        if document.persistable_record_id() != Some(id) {
    ...
    fn insert_with_status(
        ...
        if stored_digest != record_digest.as_str() {
            return Err(StoreError::RecordConflict {
                kind,
                record_id: id.clone(),
            });
        }
        let status = if transaction.changes() == 1 {
